...
vector<face> model::getFacets()
{
    return facets;
}

vector<Vertex> model::parseFace(string input) {
...
std::vector<float> model::getMinCoords()
{
    return minCoords;
}

std::vector<float> model::getMaxCoords()
{
    return maxCoords;
}

void model::checkMin(Vertex v)
{
    float x, y, z;
    x = v.X();
    y = v.Y();
